...
包含：OpenAI 兼容模型管理、system prompt 构建、
...
def configure_agent(
    ...
    """由 main.py 启动时调用，注入所有依赖。chat_client 保留为兼容占位。"""
    global _db_file, _persona_config, _active_model
    ...
    _db_file = db_file
    _active_model = ai_provider.get_active_model("chat") or _active_model
...
def reset_active_model() -> str:
    """Provider 切换后刷新 agent 模块内缓存的 chat 模型。"""
